...
execute as @a[tag=InGame] run scoreboard players operation @s Lives -= @s wrong
...
execute as @a[tag=InGame] if score @s Lives matches ..0 run tp @s 1058 111 1001 90 0

execute as @a[tag=InGame] if score @s Lives matches ..0 if score testGame puzzleSettings matches 0 run tellraw @a ["",{"text":"[","color":"white"},{"text":"Puzzle Wars","color":"dark_purple"},{"text":"] ","color":"white"},{"selector":"@s"},{"text":" was eliminated","color":"gold"}]
