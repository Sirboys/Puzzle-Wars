
team add red
team add blue
team add yellow
team add green
team add pink
team add white
team add black
team add gray

team modify red color red
team modify blue color blue
team modify yellow color yellow
team modify green color green
team modify pink color light_purple
team modify white color white
team modify black color black
team modify gray color gray

scoreboard objectives add ReJoin minecraft.custom:minecraft.leave_game

gamerule announceAdvancements false
gamerule reducedDebugInfo true
gamerule commandBlockOutput false
gamerule logAdminCommands false
gamerule doDaylightCycle false
gamerule doWeatherCycle false
gamerule doLimitedCrafting true
gamerule doMobSpawning false
gamerule doEntityDrops false
gamerule randomTickSpeed 0
gamerule spectatorsGenerateChunks false
gamerule mobGriefing false
gamerule showDeathMessages false

time set 6000
weather clear

scoreboard objectives add rndImages dummy
scoreboard objectives add rndImg dummy
scoreboard objectives add SCOREBORD dummy
scoreboard objectives add calcScore dummy
scoreboard objectives add kickCalc dummy
scoreboard objectives add Coins dummy
scoreboard objectives add coinsInGame dummy
scoreboard objectives add Lives dummy
scoreboard objectives add DefaultLives dummy
scoreboard objectives add wrong dummy
scoreboard objectives add Islands dummy
scoreboard objectives add Frames dummy
scoreboard players set 4 Numbers 4
scoreboard objectives add hpBackLevel dummy
scoreboard objectives add changePictrue minecraft.dropped:minecraft.painting
scoreboard objectives add playerBlindess minecraft.dropped:minecraft.ink_sac

say Puzzle Wars is added
